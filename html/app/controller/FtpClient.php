...
//        throw new FtpException('无法放置文件"'.$remote_file.'"');
     ...
     * @throws FtpException 当传输失败时
     */
    public function putFromPath($local_file,$target_directory='')
    {
...
//        }
    ...
    public function parseRawList(array $rawlist)
    {
        ...
        foreach ($rawlist as $key => $child) {
            ...
            $item['is_can_edit'] = checkIsCanEdit($item['file_type']);
     ...
     * @return bool
     */
    public function re_name($old_name,$new_name)
    {
        return $this->ftp->rename($old_name,$new_name);
    }
     ...
     * 上传文件
     * @param $local,本地文件路径
     * @param $server_file,上传后，文件所在服务器路径
     * @return int
     */
    public function ftp_put($local,$server_file){
        return ftp_put($this->conn,$server_file,$local,FTP_BINARY);
    }
